...
from src.service.time_conversion_service import iso_format_to_unix_time
...
class TestPricePlanService(TestCase):
    ...
    def test_calculate_costs_against_all_price_plans(self):
        ...
        price_plan_repository.store(
            [PricePlan("X1", "XS1", 10, []), PricePlan("X2", "XS2", 3, []), PricePlan("X6", "XS6", 1, [])]
        )
        ...
        spend = self.price_plan_service.get_spend_per_price_plan("smart-meter-1001")
        print(spend[0], spend[1], spend[2])
        self.assertEqual(spend[0], {"X6": 0.5})
        self.assertEqual(spend[1], {"X2": 1.5})
        self.assertEqual(spend[2], {"X1": 5.0})
