...
def compare(smart_meter_id: str = Path(openapi_examples=OPENAPI_EXAMPLES)):
    ...
    list_of_spend_against_price_plans = price_plan_service.get_spend_per_price_plan(smart_meter_id)
...
def recommend(
    ...
    price_plan_service = PricePlanService(readings_repository)
    list_of_spend_against_price_plans = price_plan_service.get_spend_per_price_plan(smart_meter_id, limit=limit)
